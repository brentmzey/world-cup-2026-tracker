...
fn get_system_config() -> Result<SystemConfig, String> {
    ...
    let config_path =
        std::env::var("APP_CONFIG_PATH").unwrap_or_else(|_| "config.json".to_string());
    ...
    let source =
        if std::env::var("APP_TENANT_ID").is_ok() || std::env::var("APP_POCKETBASE_URL").is_ok() {
...
mod tests {
    ...
    fn test_system_config_resolution() {
        ...
        assert_eq!(
            ...
            Some("https://test-pb.com".to_string())
        );
